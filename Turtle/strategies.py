...
class MyStrategy(bt.Strategy):
    params = ( ('period', 21),  ('printlog', False),('margin', 0.1), )   
    ...
    def next(self): 
        ...
        if self.CrossoverHi > 0 and self.buytime == 0 and now_rate < -0.1 * 0.01:                                 
            self.newstake = self.broker.getvalue() * 0.01 / self.ATR            
            self.buytime = 1        
            size = self.get_size(self.newstake)
            self.buy(size = self.get_size(self.newstake))        
        #加仓        
        ...
        #出场        
            ...
            self.sell(size=self.get_size(0,is_sell=1))            
            self.buytime = 0        
        #止损        
